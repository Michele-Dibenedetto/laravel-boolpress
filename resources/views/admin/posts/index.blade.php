    ...
    <div class="row justify-content-center">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <h1>Tutti i posts</h1>
                <a href="{{route("posts.create")}}" class="btn btn-primary"> {{-- {{ route('admin.posts.create') }} --}}
                    Crea nuovo post
                </a>
            </div>
            <table class="table">
                    ...
                    @foreach ($posts as $post)
                        <tr>
                            ...
                            <td>
                                ...
                                <a class="btn btn-warning btn-sm" href="{{route("posts.edit", $post->id)}}"> {{-- {{ route('admin.posts.edit', $post->id) }} --}}
                                ...
                                <form class="d-inline-block" action="{{route("posts.destroy", $post->id)}}" method="post"> {{-- {{ route('admin.posts.destroy', $post->id) }} --}}
                                    @csrf
                                    @method('DELETE')
